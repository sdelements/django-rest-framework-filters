...
class AutoFilterTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Note.objects.create(
            title="Hello Test 3", content="Test content 3", author=user1
        )
        Note.objects.create(
            title="Hello Test 4", content="Test content 4", author=user2
        )
    ...
    def test_autofilter_with_method(self):
        ...
        class Actual(FilterSet):
            ...
            author = filters.AutoFilter(
                lookups="__all__", field_name="author__username", method="filter_author"
            )
...
class RelatedFilterTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        note1 = Note.objects.create(
            title="Test 1", content="Test content 1", author=user1
        )
        note2 = Note.objects.create(
            title="Test 2", content="Test content 2", author=user1
        )
        Note.objects.create(
            title="Hello Test 3", content="Test content 3", author=user1
        )
        ...
        Page.objects.create(
            title="Second page", content="Second second.", previous_page_id=1
        )
        Page.objects.create(
            title="Third page", content="Third third.", previous_page_id=2
        )
        Page.objects.create(
            title="Fourth page", content="Fourth fourth.", previous_page_id=3
        )
        ...
        c1 = Customer.objects.create(
            name="Bob Jones", ssn="111111111", dob="1990-01-01"
        )
        c2 = Customer.objects.create(
            name="Sue Jones", ssn="222222222", dob="1990-01-01"
        )
    ...
    def test_relatedfilter_queryset_required(self):
        ...
        GET = {"author": User.objects.get(username="user2").pk}
        msg = (
            "Expected `.get_queryset()` for related filter 'NoteFilter.author' "
